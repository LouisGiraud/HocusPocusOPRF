...
int main()
{
    ...
    cout << "\n\n Test Vole+\n";
    SSVSender = new SmallSetVoleSender25519(VP.len + 1, VP.small_set_bits, VP.k_vole);
    SSVReceiver = new SmallSetVoleReceiver25519(VP.len + 1, VP.small_set_bits, VP.k_vole);
    ...
    auto recverThread = std::thread([&]()
        ...
        Vec a(N * (VP.k_vole+1)); // contains first N*k_zkp entries for the ZKP and the other N*k_vole entries are for VOLE+
        std::vector<u64> points(VP.k_vole+1); 
