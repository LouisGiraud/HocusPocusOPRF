...
public:
    ...
    VolePlus(int len, int small_set_bits, int statistical_security_bits) : len(len),
                                                                           ...
                                                                           k_vole((2 * statistical_security_bits + PRIME_BITS + small_set_bits - 1) / small_set_bits) // computes k parameter
    {
    };
        ...
        Socket &chl)
    {
        ...
        std::vector<unsigned char> buffer(PRIME_BYTES * (k_vole - 1) * (len + 1));
        co_await chl.recv(buffer);

        fe25519 vi_prime;
        for (size_t i = 0; i < k_vole - 1; i++)
        {
        ...
        co_await chl.recv(buffer);
        fe25519 cv1, check;
        cv1.unpack(buffer.data() + k_vole * PRIME_BYTES);
        ...
        std::vector<fe25519> lambda(k_vole);
        ...
        Socket &chl)
    {
        ...
        check_value.pack(buffer.data() + k_vole * PRIME_BYTES);
